...
function addToFavorites(productId) {
  ...
  const exists = favorites.value.some(
    (fav) => String(fav.customerId) === String(customerId) && Number(fav.productId) === pid
  )
  ...
  if (!exists) {
    ...
    ElMessage.info('Sản phẩm đã có trong yêu thích.')
  }
}
...
async function fetchProducts() {
  ...
  try {
    ...
    // Linh hoạt với cấu trúc payload từ backend
    const payload = data || {}
    ...
    // Lọc trùng theo id và chuẩn hoá dữ liệu sp
    const uniqueMap = new Map()
    for (const item of content) {
      if (item && item.id != null) uniqueMap.set(item.id, item)
    }
    products.value = Array.from(uniqueMap.values()).map(processProductData)

    const pageMeta = payload.page || payload.data?.page || {}
    totalItems.value =
      Number(pageMeta.totalElements) || Number(payload.totalElements) || Number(payload.total) || content.length
  } catch (err) {
...
function processProductData(product) {
  ...
  // Lấy danh sách màu duy nhất + ảnh theo màu
  const seen = new Set()
  const variants = []
  for (const d of details) {
    ...
    seen.add(key)
    const img = images.find((i) => i?.colorId === key)
    variants.push({
      colorId: key,
      colorName: d?.colorName || 'Không rõ',
      colorCode: getColorCode(d?.colorName),
      image: img?.image ? `data:image/jpeg;base64,${img.image}` : null,
    })
  }

  const activeImage = variants.length > 0 && variants[0].image ? variants[0].image : PLACEHOLDER_IMG

  return { ...product, variants, activeImage }
}
...
function openQuickViewModal(product) {
  ...
  if (product.variants?.length) {
    ...
    quickViewActiveImage.value = product.activeImage || PLACEHOLDER_IMG

    // Nếu chỉ có 1 detail và không có color/size, chọn luôn
    if (product.productDetails?.length === 1) {
      quickViewSelectedSize.value = product.productDetails[0]?.sizeName || null
    } else {
...
function findSelectedProductDetail() {
  ...
  // Trường hợp chỉ có 1 detail, không phân loại theo màu/size
  if (
    sp.productDetails.length === 1 &&
    !sp.productDetails[0].colorId &&
    !sp.productDetails[0].sizeName
  ) {
...
function getStockForSize(size) {
  const sp = selectedProduct.value
  if (!sp) return 0
  const d = sp.productDetails?.find(
    (x) => x.colorId === quickViewSelectedColorId.value && x.sizeName === size
  )
...
// New helpers to avoid min>max errors in InputNumber
...
// --- GIỎ HÀNG / MUA NGAY ---
function requiresSizeSelection(sp) {
  const namedSizes = new Set((sp.productDetails || []).map((d) => d.sizeName).filter(Boolean))
  return namedSizes.size > 0
}
...
function handleAddToCartInModal() {
  ...
  if (!detail) {
    ElMessage.error('Không tìm thấy chi tiết sản phẩm phù hợp. Vui lòng chọn lại màu và kích thước.')
    return false
  }
  ...
  if (quickViewQuantity.value > Number(detail.quantity || 0)) {
    ElMessage.warning(`Số lượng tồn kho không đủ, chỉ còn ${detail.quantity} sản phẩm. Bạn có thể đặt trước nếu muốn mua thêm.`)
    openPreOrderDialog()
    return false
  }
...
function handleBuyNowInModal() {
  const ok = handleAddToCartInModal()
...
function changeProductImage(product, newImage) {
  if (product && newImage) {
    ...
    if (idx !== -1) products.value[idx].activeImage = newImage
  }
}
...
function isHexColor(str) {
  return /^#([0-9A-F]{3}){1,2}$/i.test(String(str || ''))
}
...
function sizeComparator(a, b) {
  ...
  // Nếu là số (ví dụ size giày 36, 37, ...)
...
watch(preOrderDialogVisible, (opened) => {
  if (opened && selectedProduct.value) {
    // Clone sâu: tránh sửa state của quick view
...
function handlePreOrderConfirm() {
  ...
  apiClient
    ...
    .then(() => {
      ElMessage.success(
        `Đã đặt trước ${preOrderQuantity.value} sản phẩm ${preOrderItem.value.productName} (Màu: ${d.colorName}, Size: ${d.sizeName}). Chúng tôi sẽ liên hệ lại với bạn sớm nhất.`
      )
      preOrderDialogVisible.value = false
    })
    .catch((e) => {
      console.error('Lỗi khi đặt trước:', e)
