          ...
          <el-card shadow="never">
            ...
            <div v-if="invoiceDetails">
              ...
              <div class="mt-2">
                ...
                <span class="fs-5 text-success ms-2 fw-bold">{{ formatCurrency(changeAmount) }}</span>
              </div>
    ...
    >
      <el-form label-position="top">
        ...
        <el-form-item label="Số lượng:">
          ...
          />
          <small v-if="maxQuantity > 0" class="form-text text-muted ms-3">Tối đa có thể chọn: {{ maxQuantity }}</small>
...
import { ArrowLeft, Plus, Delete, Check, Search, User, CircleClose, Select } from '@element-plus/icons-vue';
...
const openProductDialog = async (product) => {
  ...
  try {
    const { data: attrs } = await apiClient.get( // THAY ĐỔI
...
const deleteCartItem = async (invoiceDetailId) => {
  try {
    await ElMessageBox.confirm('Bạn có chắc chắn muốn xóa sản phẩm này khỏi giỏ hàng?', 'Xác nhận xóa', {
...
const cancelInvoice = async () => {
  try {
    await ElMessageBox.confirm('Bạn có chắc chắn muốn hủy hóa đơn này không? Hành động này không thể hoàn tác.', 'Xác nhận hủy', {
...
const applyVoucher = async (voucherCode) => {
  ...
  try {
    const response = await apiClient.post( // THAY ĐỔI
      `/admin/counter-sales/${invoiceId}/apply-voucher`, null, { params: { voucherCode } }
    );
...
watch(searchTerm, debounce(() => fetchProducts(1), 300));
