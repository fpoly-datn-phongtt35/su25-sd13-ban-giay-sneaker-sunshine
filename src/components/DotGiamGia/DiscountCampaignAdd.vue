...
const dateRangeStep3 = computed({ get() { return dateRange.value }, set(v) { dateRange.value = v } })
const dateError = ref('')
...
const normalizeProduct = (row) => {
  const productText = textOf(row, ['productText']) || textOf(row, ['productName','name']) || textOf(row, ['product.productName','product.name']) || fallbackId(row.id)
  const brandText   = textOf(row, ['brandText'])   || textOf(row, ['brandName']) || textOf(row, ['brand.brandName','brand.name']) || textOf(row, ['product.brand.brandName','product.brand.name']) || fallbackId(row.brandId)
  return { ...row, productText, brandText }
}
const normalizeDetail = (row) => {
  const productText = textOf(row, ['productText']) || textOf(row, ['productName','product.productName','product.name']) || fallbackId(row.productId)
  const brandText   = textOf(row, ['brandText'])   || textOf(row, ['brandName']) || textOf(row, ['brand.brandName','brand.name']) || textOf(row, ['product.brand.brandName','product.brand.name']) || fallbackId(row.brandId)
  const colorText   = textOf(row, ['colorText'])   || textOf(row, ['colorName','color.colorName','color.name']) || fallbackId(row.colorId)
  const sizeText    = textOf(row, ['sizeText'])    || textOf(row, ['sizeName','size.sizeName','size.name','sizeValue']) || fallbackId(row.sizeId)
...
const onProductSelectionChange = (rows) => {
  ...
  if (!activeProductId.value || !selectedProductIds.value.includes(activeProductId.value)) {
    ...
    if (first) setActiveProduct(first)
      ...
      details.value = []; detailsTotal.value = 0
    }
  } else {
...
const fetchDetails = async () => {
  if (!activeProductId.value) { details.value = []; detailsTotal.value = 0; return }
  ...
  try {
    ...
    const { data } = await apiClient.get(`/admin/products/${activeProductId.value}/details`, { params })
    const content = data?.content || []
...
const validateFinal = () => {
  ...
  if (!form.name?.trim() || form.discountPercentage === '' || form.discountPercentage === null) {
    ElMessage.error('Thiếu tên chiến dịch hoặc % giảm.'); if (activeStep.value === 0) return false
  }
  ...
  if (selectedProductIds.value.length === 0 && selectedDetailIds.value.length === 0) {
    ElMessage.warning('Chọn ít nhất 1 SP hoặc 1 SPCT.'); if (activeStep.value === 1) return false
  }
  return true
}
...
const createCampaign = async () => {
  ...
  try {
    await apiClient.post('/admin/campaigns', payload, { headers: { 'Content-Type': 'application/json' } })
    ElMessage.success('Tạo đợt giảm giá thành công!')
    router.push('/discount-campaigns')
  } catch (e) {
    console.error('Create campaign error:', { status: e?.response?.status, data: e?.response?.data, payload })
