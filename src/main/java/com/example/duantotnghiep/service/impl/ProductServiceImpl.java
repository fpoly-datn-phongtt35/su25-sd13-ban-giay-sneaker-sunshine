...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductResponse createProduct(ProductRequest request) {
        // 1. Convert request -> entity (dùng MapStruct, ảnh và category xử lý riêng)
        ...
        if (request.getProductImages() != null && !request.getProductImages().isEmpty()) {
            List<ProductImage> imageList = request.getProductImages().stream().map(file -> {
                try {
                    ...
                    // Ném lỗi với thông báo i18n và chi tiết từ ngoại lệ
                    throw new RuntimeException("Lỗi ảnh: " + e); // Đưa ngoại lệ vào trong RuntimeException
                }
            }).collect(Collectors.toList());

            product.setProductImages(imageList); // cascade persist
        }

        // 3. Xử lý ProductDetail
        if (request.getProductDetails() != null && !request.getProductDetails().isEmpty()) {
            ...
            details.forEach(detail -> {
                detail.setProduct(product); // để tạo mối quan hệ đúng
            ...
            product.setProductDetails(details); // cascade persist
        }


        // 3. Lưu Product (cần ID để liên kết ProductCategory)
        Product savedProduct = productRepository.save(product);

        // 4. Lấy danh sách Category duy nhất 1 lần
        List<Category> categories = categoryRepository.findAllByIdInAndStatus(request.getCategoryIds(), 1);

        if (categories.size() != request.getCategoryIds().size()) {
            throw new RuntimeException("Không thấy category .");
        }

        // 5. Tạo danh sách ProductCategory để saveAll
        ...
        response.setProductDetails(
                savedProduct.getProductDetails().stream()
...
// Trả về kết quả
        ...
        List<CategoryResponse> categoryResponses = activeProductCategories.stream()
